...
def main(df1=None):
    ...
    # Synthetic data should live in a directory from the top level of the directory in
    # development_files\slippage_data
    #  If you do not have this directory, create one and put your synthetically generated data in there as a .csv file
    #  and specify the name below with '.csv' on the end
    synthetic_data_generated = True
    synthetic_data_file_name = "Synthetic_Base.csv"

    # If synthetic_data_generated = False, then syntheic data will be generated by reading in training data
    # This "historical" flag indicates if you want to use that data as is or if you want to ust it
    # to generate random consignments using that data. Set to True if you want to do this.
    historical = False


    # Flag to indicate if you want to randomly generate consignments every replication
    # Set to True if you want, for each replication in num_replications, to randomly generate new consignments
    randomly_generate_consignments_per_rep = False

    # Flag to indicate if you want to randomly generate consignments every replication
    # Set to True if you want, for each replication in num_replications, to used a fixed set of consignments
    run_with_fixed_consignments = True
    ...
    rep_data_dir = data_dir / "rep_synthetic_data"
    ...
    ###########################################################################################################
    # DO NOT RANDOMLY GENERATE CONSIGNMENTS EVERY REPLICATION
    ###########################################################################################################
    if run_with_fixed_consignments:
        if synthetic_data_generated:
            synth_data = pd.read_csv(data_dir / synthetic_data_file_name)
        ...
        print(f'\nTIMING SUMMARY FOR LOADING/GENERATING SYNTHETIC DATA')
        print(f'   Total Time (seconds): {total_time_seconds}')
        print(f'   Total Time (minutes): {time_minutes}')

        config["consignment"]["input_file"]["file_name"] = str(data_dir / synthetic_data_file_name)
        ...
        for scenario in scenarios:
            #Setting actual paramter vaLues
            for key in res.keys():
    ...
    if randomly_generate_consignments_per_rep:

        ### Synthetic data generation
        # Master RNG from global seed
        master_rng = np.random.default_rng(1)

        # Precompute a seed for each replication index
        # So replication i has the same base seed across all scenarios
        consigment_replication_seeds = master_rng.integers(2 ** 63 - 1, size=num_replications)

        scenario_rngs = [
            np.random.default_rng(int(consigment_replication_seeds[i]))
        ...
        for sim_rep in range(num_replications):
            synthetic_data_generator = SyntheticConsignmentDataGenerator(config=config,
                                                                         producer_group_mapping=producer_group_mapping,
                                                                         input_data_file=pis_data_train,
                                                                         rng=scenario_rngs[sim_rep])
            print(f'\nStarting Consignment Generation Process of {num_consignments_to_simulate} Requested Consignments...')
            synth_data = synthetic_data_generator.generate_from_input_data(
                n_consignments=num_consignments_to_simulate,
                sampling_method="sequential"
            ...
            synth_out_path = rep_data_dir / f"Synthetic_temp_rep_{sim_rep}.csv"
            config["consignment"]["input_file"]["file_name"] = str(synth_out_path)
            synth_data.loc[:, 'Row_ID'] = 'CR-' + (synth_data.index + 1).astype(str)
            ...
            # Read in training data used to create producer and importer top variables
            dt_train = pd.read_csv(pis_data_train2)
            # dt_train = pd.read_csv(pis_data_train)
        ...
        if not compliance_table_dir.exists():
            ...
            if not files:
                ...
                for filename in files:
                    ...
                    for sim_rep in range(num_replications):
                        synth_out_path = rep_data_dir / f"Synthetic_temp_rep_{sim_rep}.csv"
                        synth_data = pd.read_csv(synth_out_path)
                        ...
                        if "prod_group_name" in temp_compliance_table.columns:
                            ...
                            print(
                                f"[Producer groups vs {comp_col}] 'Reference' count before: {before_ref}, after: {after_ref}")
                            ...
                            # Create a DataFrame for these values
                            replaced_df = pd.DataFrame(replaced_values, columns=["Replaced_Producer_Group"])
                            replaced_df.to_csv(data_dir / "replaced_producer_group_from_PRODUCER_GROUP_TOP.csv",
                                               index=False)
                        ...
                        if "IMPORTER_NAME_TOP" in temp_compliance_table.columns:
                            ...
                            print(
                                f"[Importer names vs {comp_col}] 'Reference' count before: {before_ref}, after: {after_ref}")
                        ...
                        #synth_data.to_parquet(rep_data_dir / "Synthetic_Base_Use.parquet", compression='snappy', index=False)
                        synth_data.to_csv(rep_data_dir / f"Synthetic_Base_Use_rep_{sim_rep}.csv", index=False)
                    ...
                    if detailed_bool:
                        ...
                        scenario_results = [(result, config) for result, config in scenario_results_raw]
                    config_columns = ['contamination/contamination_unit',
                                      'contamination/contamination_rate/distribution',
                                      'contamination/contamination_rate/value', 'contamination/arrangement',
                                      'inspection/sample_strategy', 'inspection/proportion/value',
                                      'inspection/tolerance_level', 'name']
                    result_columns = list(vars(scenario_results[0][0]).keys())
                    ...
                    if detailed_bool:
                        ...
                        if inspection_unit_records:
                            ...
                            if len(str(full_path)) > MAX_PATH_LENGTH:
                                ...
                                # Truncate and add hash
                                truncated_name = clean_filename[
                                    :max(0, max_name_length - 9)]  # -9 for underscore + hash
                                clean_filename = f"{truncated_name}_{hash_suffix}"
